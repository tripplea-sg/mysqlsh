...
def _check_local_role():
    ...
    result = _run_sql("show status like 'group_replication_primary_member'",False)
    result = _run_sql("select concat(member_host,':',member_port) from performance_schema.replication_group_members where channel_name='group_replication_applier' and member_id='" + result[0][1] + "'",False)
    
    if len(result) == 0:
        return "OFFLINE"

    if result[0][0] == hostname + ':' + str(port):
        return "PRIMARY"
    else:
        return "SECONDARY"
...
def _get_gtid():
    result = _run_sql("select @@gtid_executed",False)
    return result[0][0]
...
def _stop_other_replicas():
    if len(_list_all_channel()) != 0:
        for channelName in _list_all_channel():
            result = _run_sql("stop slave for channel '" + channelName[0] + "'", False)
...
def _set_grseed_replicas(gr_seed, clusterAdmin):
    ...
    _run_sql("set global group_replication_group_seeds='" + gr_seed + "'",False)
...
def _create_or_add(ops, connectionStr, group_replication_group_name, group_replication_group_seeds):
    ...
    result = _run_sql('set global super_read_only=off',False)
    _install_plugin("group_replication", "group_replication.so")
    result = _run_sql("set global group_replication_group_name='" + group_replication_group_name + "'",False)
    result = _run_sql("set global group_replication_start_on_boot='ON'",False)
    result = _run_sql("set global group_replication_bootstrap_group=off",False)
    result = _run_sql("set global group_replication_recovery_use_ssl=1",False)
    result = _run_sql("set global group_replication_ssl_mode='REQUIRED'",False)
    result = _run_sql("set global group_replication_ip_whitelist = '" + grAllowList + "'",False)
    _run_sql("set global auto_increment_increment = 7",False)
    _run_sql("set global auto_increment_offset = 20",False)
    _run_sql("set global group_replication_allow_local_disjoint_gtids_join = OFF",False)
    _run_sql("set global group_replication_allow_local_lower_version_join = OFF",False)
    _run_sql("set global group_replication_auto_increment_increment = 7",False)
    _run_sql("set global group_replication_components_stop_timeout = 31536000",False)
    _run_sql("set global group_replication_compression_threshold = 1000000",False)
    _run_sql("set global group_replication_enforce_update_everywhere_checks = OFF",False)
    _run_sql("set global group_replication_exit_state_action = READ_ONLY",False)
    _run_sql("set global group_replication_flow_control_applier_threshold = 25000",False)
    _run_sql("set global group_replication_flow_control_certifier_threshold = 25000",False)
    _run_sql("set global group_replication_flow_control_mode = QUOTA",False)
    _run_sql("set global group_replication_gtid_assignment_block_size = 1000000",False)
    _run_sql("set global group_replication_member_weight = 50",False)
    _run_sql("set global group_replication_poll_spin_loops = 0",False)
    _run_sql("set global group_replication_recovery_complete_at = TRANSACTIONS_APPLIED",False)
    _run_sql("set global group_replication_recovery_reconnect_interval = 60",False)
    _run_sql("set global group_replication_recovery_retry_count = 10",False)
    _run_sql("set global group_replication_recovery_ssl_verify_server_cert = OFF",False)
    _run_sql("set global group_replication_single_primary_mode = ON",False)
    _run_sql("set global group_replication_transaction_size_limit = 0",False)
    _run_sql("set global group_replication_unreachable_majority_timeout = 0",False)
    _run_sql("set global super_read_only = ON",False)
    hostname = _check_report_host()
    result = _run_sql("set global group_replication_local_address='{}:{}'".format(hostname, int(shell.parse_uri(shell.get_session().get_uri())['port'])+10),False)
...
def status(session=None):
    ...
    result = _run_sql("show status like 'group_replication_primary_member'",False)
    print("PRIMARY member : " + result[0][1] + "\n")
    print("Local ROLE is \033[1m" + _check_local_role() + "\033[0m\n")
...
def setPrimaryInstance(connectionStr):
    ...
    if _check_local_role() != "PRIMARY":
        result = ("show status like 'group_replication_primary_member'",False)
        current_primary = _run_sql("select concat(member_host,':',member_port) from performance_schema.replication_group_members where channel_name='group_replication_applier' and member_id='" + result[0][1] + "'", False)
        try:
    ...
    try:
        ...
        if len(_list_all_channel()) != 0:
            for channel_name in _list_all_channel():
                ...
                result = _run_sql("stop slave for channel '" + channel_name[0] + "'", False)
        ...
        if len(start_channel_name) != 0:
            for channel_name in start_channel_name:
                ...
                result = _run_sql("start slave for channel '" + channel_name[0] + "'", False)
    except:
    ...
    if p == "Y":
        ...
        try:
            ...
            if check != "0":
                ...
                result = _run_sql("reset global group_replication_group_name",False)
                result = _run_sql("reset global group_replication_start_on_boot",False)
                result = _run_sql("reset global group_replication_bootstrap_group",False)
                result = _run_sql("reset global group_replication_local_address",False)
                result = _run_sql("reset global group_replication_group_seeds",False)
                try:
...
def syncLocalMembers():
    ...
    if len(result) != 0:
        ...
        result = _run_sql("set global group_replication_group_seeds='" + group_replication_group_members[1:] + "'",False)
...
def GRToIC(clusterName):
    ...
    _drop_ic_metadata()
    dba.create_cluster(clusterName, {"adoptFromGR":True, "force":True})
...
@plugin_function("group_replication.setReplicationCluster")
def setReplicationCluster(conn):
    global clusterAdmin
    global clusterAdminPassword
    ...
    if checkInstanceConfiguration():
        ...
        try:
            ...
            shell.set_session(y)
            cluster_nodes = _run_sql("select a.cluster_name from mysql_innodb_cluster_metadata.clusters a, mysql_innodb_cluster_metadata.instances b order by b.instance_id", False)
        ...
        try:
            result = _run_sql("stop slave for channel '" + PrimaryClusterName + "'", False)
        except:
            print("\n\033[1mINFO:\033[0m Create a new replication to PRIMARY Cluster \033[0m\n")    
        
        result = _run_sql("change master to master_host='" + primaryHost + "', master_port=" + primaryPort + ", master_user='" + clusterAdmin + "', master_password='" + clusterAdminPassword + "', master_auto_position=1, master_connect_retry=3, master_retry_count=3 for channel '" + PrimaryClusterName + "'", False)
        
        result = _run_sql("start slave for channel '" + PrimaryClusterName + "'", False)
...
def setPrimaryCluster(conn):
    ...
    if checkInstanceConfiguration():
        ...
        try:
            ...
            cluster_nodes = _run_sql("select a.cluster_name from mysql_innodb_cluster_metadata.clusters a, mysql_innodb_cluster_metadata.instances b order by b.instance_id", False)
            PrimaryClusterName = cluster_nodes[0][0] 
            cluster_nodes = _run_sql("SELECT concat(member_host, ':', member_port) FROM performance_schema.global_status JOIN performance_schema.replication_group_members WHERE variable_name = 'group_replication_primary_member' AND member_id=variable_value;", False)
            primaryHost = shell.parse_uri(cluster_nodes[0][0])['host']
            primaryPort = str(shell.parse_uri(cluster_nodes[0][0])['port'])
        except:
            shell.set_session(x)
            print("\n\033[1mERROR:\033[0m Primary Cluster Host cannot be contacted \033[0m\n")
            return
...
@plugin_function("group_replication.showInstanceConfiguration")
def showInstanceConfiguration():
    if not checkInstanceConfiguration():
        print('\n\033[1mERROR:\033[0m Instance is not a Group Replication or User is not a cluster admin\n')
        return
    ...
    if v_continue == 0:
        print('\n\033[1mERROR:\033[0m This has to be executed on the Group Replication\n')
        return

    result = _run_sql("show variables like 'group_replication_group_seeds'",False)
    print("group_replication_group_seeds='" + result[0][1] + "'")
    result = _run_sql("show variables like 'group_replication_group_name'",False)
    print("group_replication_group_name='" + result[0][1] + "'")
    result = _run_sql("show variables like 'group_replication_start_on_boot'",False)
    print("group_replication_start_on_boot=" + result[0][1])
    result = _run_sql("show variables like 'group_replication_bootstrap_group'",False)
    print("group_replication_bootstrap_group=" + result[0][1])
    result = _run_sql("show variables like 'group_replication_recovery_use_ssl'",False)
    print("group_replication_recovery_use_ssl=" + str(result[0][1]))
    result = _run_sql("show variables like 'group_replication_ssl_mode'",False)
    print("group_replication_ssl_mode=" + result[0][1])
    result = _run_sql("show variables like 'group_replication_ip_whitelist'",False)
    print("group_replication_ip_whitelist='" + result[0][1] +"'")
    result = _run_sql("show variables like 'group_replication_local_address'",False)
    print("group_replication_local_address='" + result[0][1] + "'")
    print("auto_increment_increment = 7")
    print("auto_increment_offset = 20")
    print("group_replication_allow_local_disjoint_gtids_join = OFF")
    print("group_replication_allow_local_lower_version_join = OFF")
    print("group_replication_auto_increment_increment = 7")
    print("group_replication_components_stop_timeout = 31536000")
    print("group_replication_compression_threshold = 1000000")
    print("group_replication_enforce_update_everywhere_checks = OFF")
    print("group_replication_exit_state_action = READ_ONLY")
    print("group_replication_flow_control_applier_threshold = 25000")
    print("group_replication_flow_control_certifier_threshold = 25000")
    print("group_replication_flow_control_mode = QUOTA")
    print("group_replication_force_members =")
    print("group_replication_gtid_assignment_block_size = 1000000")
    print("group_replication_member_weight = 50")
    print("group_replication_poll_spin_loops = 0")
    print("group_replication_recovery_complete_at = TRANSACTIONS_APPLIED")
    print("group_replication_recovery_reconnect_interval = 60")
    print("group_replication_recovery_retry_count = 10")
    print("group_replication_recovery_ssl_ca =")
    print("group_replication_recovery_ssl_capath =")
    print("group_replication_recovery_ssl_cert =")
    print("group_replication_recovery_ssl_cipher =")
    print("group_replication_recovery_ssl_crl =")
    print("group_replication_recovery_ssl_crlpath =")
    print("group_replication_recovery_ssl_key =")
    print("group_replication_recovery_ssl_verify_server_cert = OFF")
    print("group_replication_single_primary_mode = ON")
    print("group_replication_transaction_size_limit = 0")
    print("group_replication_unreachable_majority_timeout = 0")
